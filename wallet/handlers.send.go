...
func SendStart(c echo.Context) error {
	...
	// Call the modofied Tapd service to write our sighash.hex file
	_, err = tapd.SignVirtualPSBT(cfg.TapdHost, cfg.TapdMacaroon, fundedPsbt.FundedPSBT)
...
func SendComplete(c echo.Context) error {
	...
	signedPsbt, err := tapd.SignVirtualPSBT(cfg.TapdHost, cfg.TapdMacaroon, payload.PSBT)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{
			"error": err.Error(),
		})
